...
void MoveAction::push_command(char cmd)
{
...
void MoveAction::task()
{
    for (;;)
    {
        ...
        if (xQueueReceive(commandQueue, &cmd, portMAX_DELAY) == pdPASS)
        {
            ...
            if (cmd == 'S' || cmd == 'F')
            { // ST_FULL: Đi thẳng 1 ô (300mm)
                float dist = FULL_CELL;
                ctrl::AccelDesigner ad(METRIC_JERK * 1000.0f, METRIC_ACCEL * 1000.0f, METRIC_SEARCH_SPEED * 1000.0f, 0, 0, dist);
            ...
            else if (cmd == 'R' || cmd == 'L' || cmd == 'B')
            {
                ...
                // 1. Tiến nửa ô (150mm)
                ctrl::AccelDesigner ad_in(METRIC_JERK * 1000.0f, METRIC_ACCEL * 1000.0f, METRIC_SEARCH_SPEED * 1000.0f, 0, 0, HALF_CELL);
                ...
                ctrl::AccelDesigner ad_rot(SPIN_TURN_JERK, SPIN_TURN_ALPHA, SPIN_TURN_OMEGA, 0, 0, angle);
                for (float t = 0; t < ad_rot.t_end(); t += Ts)
                {
                    // back_gain: kéo robot về gốc khi xoay (tránh trôi)
                    float delta = speedCtrl.est_p.x * cos(-speedCtrl.est_p.th) - speedCtrl.est_p.y * sin(-speedCtrl.est_p.th);
                ...
                for (int i = 0; i < 2000; i++) // Max 2s timeout
                {
                    ...
                    int_error += error * Ts;
                    speedCtrl.set_target(-delta * back_gain, Kp_settle * error + Ki_settle * int_error);
                    vTaskDelay(pdMS_TO_TICKS(1));
                    if (fabsf(Kp_settle * error) + fabsf(Ki_settle * int_error) < 0.1f * PI)
                        break;
                }
                speedCtrl.set_target(0, 0, 0, 0);
                vTaskDelay(pdMS_TO_TICKS(100)); // Dừng nhẹ
                ...
                // 3. Tiến nửa ô (150mm)
                ctrl::AccelDesigner ad_out(METRIC_JERK * 1000.0f, METRIC_ACCEL * 1000.0f, METRIC_SEARCH_SPEED * 1000.0f, 0, 0, HALF_CELL);
...
// Hàm replace chuỗi
static int replace(std::string &src, const std::string &from, const std::string &to)
{
    if (from.empty())
        return 0;
...
void MoveAction::execute_fast_run(std::string fast_path)
{
    ...
    // Giới hạn động học góc PHẢI được scale xuống để giữ nguyên vận tốc tuyến tính!
    // Nếu không scale, robot sẽ chạy cua nhanh gấp 3.33 lần Kerise (lên tới >2m/s) gây văng xe!
    ...
    // Constructor #1: Shape(total, y_curve_end, x_adv, dddth_max, ddth_max, dth_max)
    // y_curve_end = curve Y displacement (auto-generates curve, straight_prev/post, v_ref)
    ctrl::slalom::Shape shapeF45(ctrl::Pose(90.0f * K, 45.0f * K, PI / 4.0f), 30.0f * K, 0, jw_max, aw_max, w_max);
    ctrl::slalom::Shape shapeF90(ctrl::Pose(90.0f * K, 90.0f * K, PI / 2.0f), 70.0f * K, 0, jw_max, aw_max, w_max);
    ctrl::slalom::Shape shapeF135(ctrl::Pose(45.0f * K, 90.0f * K, 3.0f * PI / 4.0f), 80.0f * K, 0, jw_max, aw_max, w_max);
    ctrl::slalom::Shape shapeF180(ctrl::Pose(0.0f, 90.0f * K, PI), 90.0f * K, 24.0f * K, jw_max, aw_max, w_max);
    ctrl::slalom::Shape shapeFV90(ctrl::Pose(45.0f * sqrt_2 * K, 45.0f * sqrt_2 * K, PI / 2.0f), 48.0f * K, 0, jw_max, aw_max, w_max);
    ctrl::slalom::Shape shapeFS90(ctrl::Pose(45.0f * K, 45.0f * K, PI / 2.0f), 44.0f * K, 0.0f, jw_max, aw_max, w_max);

    const float TAIL_OFFSET = 48.0f; // khoảng cách từ đuôi (áp tường) tới tâm robot, mm
    float straight_acc = HALF_CELL - TAIL_OFFSET;
    float current_v = 0.0f;
    ctrl::TrajectoryTracker::Gain tt_gain;
    // Hạ omega_n xuống 1.5. Xe 300mm có quán tính rất lớn.
    // Nếu để 5.0, Kx = 25, chỉ cần lệch quỹ đạo 2.8cm là RefV sẽ vọt lên 1500mm/s gây trượt bánh và mất lái!
    ...
    auto run_straight = [&](float dist, float v_end)
    {
        ...
        traj.reset(j_run, a_run, v_run, current_v, v_end, dist);
        ...
        {
            traj.update(s, t);
            const auto ref = tt.update(speedCtrl.est_p,
                                       ctrl::Polar(speedCtrl.est_v, speedCtrl.est_w),
                                       ctrl::Polar(speedCtrl.est_dv, speedCtrl.est_dw),
                                       s);
    ...
    auto run_slalom = [&](ctrl::slalom::Shape &shape, bool is_right, bool reverse = false)
    {
        ...
        straight_acc += !reverse ? traj.getShape().straight_prev : traj.getShape().straight_post;
        run_straight(straight_acc, v_slalom);
        straight_acc = 0.0f;
    ...
    for (char c : fast_path)
    {
        ...
        case 'w':
            ...
            break; // ST_DIAG
        case 'z':
            run_slalom(shapeF45, false, false);
            break; // F45_L
        case 'c':
            run_slalom(shapeF45, true, false);
            break; // F45_R
        case 'Z':
            run_slalom(shapeF45, false, true);
            break; // F45_LP (same shape, reverse=true)
        case 'C':
            run_slalom(shapeF45, true, true);
            break; // F45_RP (same shape, reverse=true)
        case 'q':
            run_slalom(shapeF90, false);
            break; // F90_L
        case 'Q':
            run_slalom(shapeF90, true);
            break; // F90_R
        case 'p':
            run_slalom(shapeFV90, false);
            break; // FV90_L
        case 'P':
            run_slalom(shapeFV90, true);
            break; // FV90_R
        case 'L':
            run_slalom(shapeFS90, false);
            break; // FS90_L
        case 'R':
            run_slalom(shapeFS90, true);
            break; // FS90_R
        case 'a':
            run_slalom(shapeF135, false, false);
            break; // F135_L
        case 'd':
            run_slalom(shapeF135, true, false);
            break; // F135_R
        case 'A':
            run_slalom(shapeF135, false, true);
            break; // F135_LP (same shape, reverse=true)
        case 'D':
            run_slalom(shapeF135, true, true);
            break; // F135_RP (same shape, reverse=true)
        case 'u':
            run_slalom(shapeF180, false);
            break; // F180_L
        case 'U':
            run_slalom(shapeF180, true);
            break; // F180_R
        }
    }
