...
static constexpr float RotationRadius = 38.7619f;  // mm (khoảng cách tâm xe đến bánh)
...
static constexpr float ScalePulsesToMm = (PI * WheelDiameter) / (CountsPerRev * GearRatio); // ~0.01994 mm/count
...
static constexpr float BatteryDividerRatio = BatteryR2 / (BatteryR1 + BatteryR2);
static constexpr float AdcFSR = 4095.0f;
static constexpr float AdcRefVolts = 3.30f;
static constexpr float BatteryMultiplier = (AdcRefVolts / AdcFSR) / BatteryDividerRatio;
static constexpr float BatteryMinVoltage = 6.4f;   // Ngưỡng pin 2S cạn (3.2V/cell)
static constexpr float BatteryNominalVoltage = 7.4f;

/* Motor Models & System Identification (mm/s/V & rad/s/V) */
static constexpr ctrl::FeedbackController<ctrl::Polar>::Model SpeedControllerModel = {
    .K1 = ctrl::Polar(447.803f, 1.83f),
    .T1 = ctrl::Polar(0.53757f, 0.29213f),
};

static constexpr ctrl::FeedbackController<ctrl::Polar>::Gain SpeedControllerGain = {
    .Kp = ctrl::Polar(0.024f, 2.0f),
    .Ki = ctrl::Polar(0.045f, 6.0f),
    .Kd = ctrl::Polar(0.0f, 0.0f),
};

/* Velocity Estimation Filter (alpha = 0.95 for translation, 1.0 for gyro rotation) */
...
static constexpr float a_fast = 8000.0f;       // mm/s^2
static constexpr float j_fast = 300000.0f;     // mm/s^3
