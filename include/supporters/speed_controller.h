...
public:
  ...
  volatile float ref_th_local =
      0; // Local reference heading for exact Kerise failsafe
  ...
  void set_target(float v, float w, float dv = 0, float dw = 0) {
    ...
    ref_v = constrain(v, -20000.0f, 20000.0f);
  ...
  void update_pose(const ctrl::Pose &p) {
    ...
    est_p = p;
    ref_th_local += delta_th; // Rotate the failsafe reference by the exact same
                              // amount to maintain invariant error!
    taskEXIT_CRITICAL();
  }
  void reset();
